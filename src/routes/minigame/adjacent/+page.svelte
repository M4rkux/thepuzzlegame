...
<div class="container mx-auto">
	<div class="p-4 flex flex-col text-center items-center">
		<h1 class="mb-2">Adjacentes</h1>
		<h2 class="mb-2">Acenda todos os blocos</h2>
		<p class="mb-4">O bloco clicado acende os adjacentes</p>
						...
						{#each Array(3) as _, col}
							<td class="w-1/3 h-1/3 p-2">
								<button
									...
									class="square {!isWinner ? 'clickable' : 'cursor-default'} {squares[row][col]
										? 'glow'
										: ''}"
								></button>
							</td>
		...
		{#if isWinner}
			<h4 class="mt-4 text-green-500 text-3xl">Parabéns, você conseguiu!</h4>
		{/if}
	</div>
</div>
